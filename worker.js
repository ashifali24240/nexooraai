...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/health" && request.method === "GET") {
      return json({ ok: true, service: "Nexora AI", ai: !!env.AI, model: MODEL });
    }
    ...
    if (url.pathname === "/api/chat") {
      ...
      try {
        ...
        const mode = SYSTEM_PROMPTS[body?.mode] ? body.mode : "fast";
        if (!message) return json({ error: "Message required" }, 400);
        if (message.length > 12000) return json({ error: "Message is too long (max 12,000 characters)." }, 413);
        ...
        if (!answer) return json({ error: "The AI returned no text." }, 502);
        return json({ answer, mode, model: MODEL });
      } catch (error) {
        return json({ error: "AI request failed.", detail: String(error?.message || error) }, 500);
      }
    }
